...
d=Device(device_number_p9)
print(d.info)
#按home键，进入home界面
d.press.home()
d.press.home()
#打开1号社区app
if d(text='1号社区').wait.exists(timeout=3000):
    d(text='1号社区').click()
#失败计数
fail_count=0
...
for i in range(10000):
    if d(text='一键开门').wait.exists(timeout=3000):
        d(text='一键开门').click()
        ...
        if d(resourceId='com.oecommunity.oeshop:id/iv_door_close').wait.exists(timeout=20000):
            ...
            #失败计入文本保存
            with open('d:/moblie_log.txt','a') as f:
                f.write(get_current_time()+' ')
                f.write('第'+str(i+1)+'次测试 ')
