...
def select_reply_from_goumaizixunliebiao(driver):
    try:
        print_log(u'【case】：test_suit_电商平台_26内容管理_购买咨询列表_查看或者回复评论',',')
        original_window=driver.current_window_handle
        enter_module(driver,'内容管理')
        enter_index(driver,'购买咨询列表')
...
def delete_reply_from_goumaizixunliebiao(driver):
    try:
        print_log(u'【case】：test_suit_电商平台_26内容管理_购买咨询列表_删除评论',',')
        original_window=driver.current_window_handle
        enter_module(driver,'内容管理')
        enter_index(driver,'购买咨询列表')
        right_window=driver.current_window_handle
