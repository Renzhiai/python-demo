...
from xmindConvert.application import launch
...
Usage:
 ...
 xmind2testcase [xmindConvert] [port_num] 
...
Example:
 ...
 xmind2testcase xmindConvert                        => launch the web testcase conversion tool locally: 127.0.0.1:5001
 xmind2testcase xmindConvert 8000                   => launch the web testcase conversion tool locally: 127.0.0.1:8000
    ...
    if len(sys.argv) > 1 and sys.argv[1].endswith('.xmind'):
        ...
        if len(sys.argv) == 3 and sys.argv[2] == '-json':
            ...
            logging.info('Convert XMind file successfully: \n'
                         ...
                         testlink_xml_file,
                         zentao_csv_file)
    elif len(sys.argv) > 1 and sys.argv[1] == 'xmindConvert':
        if len(sys.argv) == 3:
            try:
                port = int(sys.argv[2])
                launch(port=port)
